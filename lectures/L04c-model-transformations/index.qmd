...
We can either formulate a new model that better fits the data, or transform the data to better fit the model. **Both methods are (practically) equivalent.**


```{r}
#| label: fig-transform-methods
...
::: fragment
**Transforming the data is easier.** Basically, we can transform the response variable and approximate the model: $$ \log_{2}(y) \sim x $$
...
where we are transforming the response variable $y$ using the **natural** logarithm, $\log(y)$, then a one-unit increase in $x$ is associated with a change of $(e^{\beta_1} - 1) \times 100\%$ in $y$.
...
#| fig-cap: "Residual diagnostics of the log-transformed model"
...
- A 1 W/m² increase in solar radiation is associated with a `r round((exp(coef(fit_log)[["Solar.R"]]) - 1) * 100, 2)`% increase in ozone concentration.
