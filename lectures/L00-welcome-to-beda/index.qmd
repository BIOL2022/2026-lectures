...
<style>
.years{display:flex;gap:28px;align-items:flex-start;}
.years .year{flex:1 1 0;display:flex;flex-direction:column;gap:12px;}
.years .year>h3{margin:0 0 4px;font-size:18px;color:#9a938b;font-weight:600;letter-spacing:.04em;}
.years .q{padding:12px 16px;background:#fffaf3;border:1px solid #ead9c9;box-shadow:0 6px 16px -12px rgba(0,0,0,.4);border-radius:0;}
.years .q .qmark{display:block;background:transparent;color:#e64626;font-family:Georgia,serif;font-size:30px;line-height:.5;margin-bottom:6px;}
.years .q p{margin:0;font-size:17px;line-height:1.4;font-style:italic;color:#3a332c;}
.years .q p strong{font-style:normal;color:#1f1f24;}
</style>

```{r}
#| results: asis
#| echo: false
raw <- readLines("assets/feedback.txt")
raw <- trimws(raw[nzchar(raw)])
p <- strsplit(raw, "|", fixed = TRUE)
yr <- sapply(p, `[`, 1)
qt <- sapply(p, function(x) paste(x[-1], collapse = "|"))
qt <- gsub("\\*\\*(.+?)\\*\\*", "<strong>\\1</strong>", trimws(qt))
out <- vapply(sort(unique(yr)), function(y) {
  cards <- sprintf('<div class="q"><span class="qmark">&ldquo;</span><p>%s</p></div>', qt[yr == y])
  sprintf('<div class="year"><h3>%s</h3>%s</div>', y, paste(cards, collapse = ""))
}, "")
cat('<div class="years">', paste(out, collapse = ""), '</div>')
```
