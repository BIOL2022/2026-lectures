...
<style>
.spotlight{position:relative;height:480px;}
.spotlight .quote{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:0 90px;background:#fff;}
.spotlight .quote .qmark, .spotlight .quote mark{background:transparent;color:#e64626;font-family:Georgia,"Times New Roman",serif;font-size:82px;line-height:.4;margin-bottom:18px;}
.spotlight .quote p{font-size:36px;line-height:1.3;font-style:italic;color:#26211c;max-width:820px;margin:0;}
.spotlight .quote p strong{font-style:normal;color:#1b1b1f;}
</style>
...
fmt <- function(x, i) {
  cls <- if (i == 1) "quote" else "quote fragment"
  sprintf('<div class="%s"><span class="qmark">&ldquo;</span><p>%s</p></div>', cls, x)
}
cat('<div class="spotlight">', paste(mapply(fmt, q, seq_along(q)), collapse = "\n"), '</div>')
```
